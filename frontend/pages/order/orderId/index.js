...
export default function Order() {
  ...
  const makePayment = async (e) => {
    ...
    if (accepted) {
      ...
      const clientName = clients.find((cli)=> cli.id === mostrarCliente);
      const pric = price;
      console.log(description, price, clientName)
      localStorage.setItem("paymentDesc", '$40.000' + ' ' + 'Pago de orden de trabajo');
      router.push("/payment");
    }
  }
      ...
      {!loading ? (
        <div className="flex max-h-[90%] min-h-[90%] h-[90%] max-w-[60%] min-w-[60%] w-[60%] bg-[#0d0f10] fixed right-[10rem] top-[3rem] rounded-md flex-col items-center py-24 px-24 pb-12">
          <div className="min-h-[80%] max-h-[80%] h-[80%] w-[100%] overscroll-contain overflow-auto">
            {params.id && (
              <h1 className="block font-medium text-3xl pb-4">
                Actualiza los datos
              </h1>
            )}
            {!params.id && (
            ...
            >
              ...
              />
              {errors.car && (
                <span className="absolute right-[7rem] after:ml-0.5 after:text-red-500 block font-medium ">
              ...
              />
              {errors.price && (
                <span className="absolute right-[7rem] top-[15rem] after:ml-0.5 after:text-red-500 block font-medium ">
                  Este campo es requerido
                </span>
              )}
              ...
              {errors.description && (
                <span className="absolute right-[7rem] top-[33rem] after:ml-0.5 after:text-red-500 block font-medium ">
                  Este campo es requerido
                </span>
              )}
              <button
                className="rounded-full w-30 bg-[#b6f09c] p-2 hover:bg-[#bbb] m-[1rem] text-[#0d0f10] font-medium"
                role="link"
                onClick={makePayment}
              >
                Gestionar pago
          ...
          <Link href={"/order/"}>
            <button className="rounded-full w-[10rem] h-[5rem] bg-[#131619] absolute bottom-[3rem] left-[3rem] hover:bg-[#bbb] m-[1rem] hover:text-[#0d0f10] font-medium">
              Volver
            </button>
          </Link>
        </div>
